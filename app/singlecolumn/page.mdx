...
<TableOfContentsItem topic='step 01 - 1/2 creating a database with sqlmodel using a lifespan' label='01 creating a database with sqlmodel using a lifespan'>
...
<TableOfContentsItem topic='step 01 - 2/2 creating a database with sqlmodel using a lifespan' label='01 - 2/2'>
...
<TableOfContentsItem topic='step 01b -  define SessionDep for  interacting with the DB' label='01b define SessionDep for  interacting with the DB'>
```python main.py
# !className(1) separator
# !diff(4:4) +
# !diff(6:7) +
...
<TableOfContentsItem topic='step 02 - 1/2 a table for notes + a POST /api/notes endpoint' label='02 a table for notes + a POST /api/notes endpoint'>
```text .gitignore
!className(1) separator
!diff(4:4) +
........
...
<TableOfContentsItem topic='step 02 - 2/2 a table for notes + a POST /api/notes endpoint' label='02 - 2/2'>
```python main.py
# !className(1) separator
# !diff(4:4) +
...
<TableOfContentsItem topic='step 02b -  GET /api/notes endpoint' label='02b GET /api/notes endpoint'>
```python main.py
# !className(1) separator
# !diff(5:5) +
...
<TableOfContentsItem topic='step 02c -  GET /api/notes/{i} endpoint' label='02c GET /api/notes/{i} endpoint'>
```python main.py
# !className(1) separator
# !diff(5:12) +
........
...
<TableOfContentsItem topic='step 03 - 1/2 expose /static/ folder with the CSS style' label='03 expose /static/ folder with the CSS style'>
```python main.py
# !className(1) separator
# !diff(4:4) +
# !className(7) separator
...
<TableOfContentsItem topic='step 03 - 2/2 expose /static/ folder with the CSS style' label='03 - 2/2'>
```css style.css
/* !diff(1:78) + */
:root {
...
<TableOfContentsItem topic='step 03b - 1/2 GET /front/notes returns templated HTML' label='03b GET /front/notes returns templated HTML'>
```python main.py
# !className(1) separator
# !diff(5:7) +
...
<TableOfContentsItem topic='step 03b - 2/2 GET /front/notes returns templated HTML' label='03b - 2/2'>
```html notes.html.j2
<!-- !diff(1:26) + -->
<!DOCTYPE html>
<html lang="en">
...
<TableOfContentsItem topic='step 04 - 1/2 redirect / to /front/notes' label='04 redirect / to /front/notes'>
```python main.py
# !className(1) separator
# !diff(5:5) +
# !className(8) separator
...
<TableOfContentsItem topic='step 04 - 2/2 redirect / to /front/notes' label='04 - 2/2'>
```html notes.html.j2
<!-- !className(1) separator -->
<!-- !diff(4:4) - -->
<!-- !diff(5:5) + -->
...
<TableOfContentsItem topic='step 05 -  PATCH /api/notes endpoint to change a note' label='05 PATCH /api/notes endpoint to change a note'>
```python main.py
# !className(1) separator
# !diff(5:5) +
# !className(8) separator
...
<TableOfContentsItem topic='step 06 - 1/2 clicking done is published to the backend' label='06 clicking done is published to the backend'>
```html notes.html.j2
<!-- !className(1) separator -->
<!-- !diff(5:5) - -->
<!-- !diff(6:7) + -->
...
<TableOfContentsItem topic='step 06 - 2/2 clicking done is published to the backend' label='06 - 2/2'>
...
<TableOfContentsItem topic='step 07 - 1/2 the UI can create a note' label='07 the UI can create a note'>
```html notes.html.j2
<!-- !className(1) separator -->
<!-- !diff(4:12) + -->
<!-- !className(16) separator -->
...
<TableOfContentsItem topic='step 07 - 2/2 the UI can create a note' label='07 - 2/2'>
...
<TableOfContentsItem topic='step 08 -  DELETE /api/notes/{i}' label='08 DELETE /api/notes/{i}'>
```python main.py
# !className(1) separator
# !diff(5:18) +
........
...
<TableOfContentsItem topic='step 09 - 1/2 the UI can delete a note' label='09 the UI can delete a note'>
```js update-backend.js
// !className(1) separator
// !diff(5:21) +
........
...
<TableOfContentsItem topic='step 09 - 2/2 the UI can delete a note' label='09 - 2/2'>
```html notes.html.j2
<!-- !className(1) separator -->
<!-- !diff(5:5) - -->
<!-- !diff(6:8) + -->
...
<TableOfContentsItem topic='step 10 -  fine-grained types NoteCreate NoteUpdate Note' label='10 fine-grained types NoteCreate NoteUpdate Note'>
```python main.py
# !className(1) separator
# !diff(3:5) -
# !diff(6:16) +
...
<TableOfContentsItem topic='step 10a -  use the new types' label='10a use the new types'>
```python main.py
# !className(1) separator
# !diff(5:5) -
# !diff(6:6) +
...
<TableOfContentsItem topic='step 11 - 1/2 the WebsocketBroadcaster class' label='11 the WebsocketBroadcaster class'>
```python main.py
# !className(1) separator
# !diff(5:10) +
........
...
<TableOfContentsItem topic='step 11 - 2/2 the WebsocketBroadcaster class' label='11 - 2/2'>
...
<TableOfContentsItem topic='step 11a -  /ws endpoint' label='11a /ws endpoint'>
```python main.py
# !className(1) separator
# !diff(5:5) +
# !className(8) separator
...
<TableOfContentsItem topic='step 11b - 1/2 inject ws-listener.js in template' label='11b inject ws-listener.js in template'>
```html notes.html.j2
<!-- !className(1) separator -->
<!-- !diff(4:4) + -->
........
...
<TableOfContentsItem topic='step 11b - 2/2 inject ws-listener.js in template' label='11b - 2/2'>
```js ws-listener.js
// !diff(1:23) +
document.addEventListener("DOMContentLoaded",
...
<TableOfContentsItem topic='step 12 -  the api endpoints broadcast the changes on WS' label='12 the api endpoints broadcast the changes on WS'>
```python main.py
# !className(1) separator
# !diff(5:5) -
# !diff(6:6) +
...
<TableOfContentsItem topic='step 13 - 1/3 handle incoming WS messages in the frontend' label='13 handle incoming WS messages in the frontend'>
```js ws-listener.js
// !className(1) separator
// !diff(5:19) +
........
...
<TableOfContentsItem topic='step 13 - 2/3 handle incoming WS messages in the frontend' label='13 - 2/3'>
```html notes.html.j2
<!-- !className(1) separator -->
<!-- !diff(5:5) + -->
........
...
<TableOfContentsItem topic='step 13 - 3/3 handle incoming WS messages in the frontend' label='13 - 3/3'>
...
<TableOfContentsItem topic='step 13b -  define id=note-{id} to the note &lt;li&gt;' label='13b define id=note-{id} to the note &lt;li&gt;'>
```html notes.html.j2
<!-- !className(1) separator -->
<!-- !diff(4:4) - -->
<!-- !diff(5:5) + -->
...
<TableOfContentsItem topic='step 13c -  CSR on note creation' label='13c CSR on note creation'>
```js clientside-rendering.js
// !className(1) separator
// !diff(2:3) -
// !diff(4:19) +
...
<TableOfContentsItem topic='step 13d -  CSR on note update' label='13d CSR on note update'>
```js clientside-rendering.js
// !className(1) separator
// !diff(4:5) -
// !diff(6:15) +
...
<TableOfContentsItem topic='step 13e -  CSR on note deletion' label='13e CSR on note deletion'>
```js clientside-rendering.js
// !className(1) separator
// !diff(4:5) -
// !diff(6:12) +
